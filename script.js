...
a();
  ...
  const products = document.querySelectorAll('.prod1');

  products.forEach(prod1 => {
      prod1.addEventListener('mousemove', (e) => {
          const rect = prod1.getBoundingClientRect();
          ...
          const offsetX = -(x - centerX) / 10; // Increase movement by decreasing divisor
          const offsetY = -(y - centerY) / 10; // Increase movement by decreasing divisor

          prod1.style.transform = `translate(${offsetX}px, ${offsetY}px)`;
      });

      prod1.addEventListener('mouseleave', () => {
          prod1.style.transform = 'translate(0, 0)';
      });
  });
